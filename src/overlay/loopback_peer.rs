...
//     remote: Weak<Mutex<LoopbackPeer<N>>>,
// }
...
// N: NominationValue
// {
//     fn send_message(&mut self, envelope:
// &super::overlay_manager::HSCPMessage) {         print!("Sending a message");
...
//     fn peer_state(&mut self) -> &mut super::peer::SCPPeerState {
//         todo!()
//     }
...
//         initiator_handle.lock().unwrap().remote =
// Arc::downgrade(&acceptor_handle);         acceptor_handle.lock().unwrap().
// remote = Arc::downgrade(&initiator_handle);         LoopbackPeerConnection {
...
//         let num_tasks_done =
// work_scheduler.borrow_mut().excecute_main_thread_tasks();         assert_eq!
// (connection.acceptor.lock().unwrap().in_queue.len(), 0);         assert_eq!
// (num_tasks_done, 1);     }
// }
